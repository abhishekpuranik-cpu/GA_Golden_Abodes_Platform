...
function refuseAnswer(appId, question, quality) {
  ...
  return {
    ...packAnswer({
      ...
      question,
      intent: 'general',
      headline: 'Cannot answer reliably — insufficient live data',
      direct: `**Cannot answer reliably:** context quality is **${quality?.score ?? 0}/100** (${reasons}). The server did not receive enough real metrics/evidence to answer “${String(question || '').slice(0, 120)}” without guessing. Open the app so data loads, then re-ask — or ask inside an app with live boards (Cashflow, Hiring, Post Sales, PreConstruction, DM).`,
      evidence: [],
