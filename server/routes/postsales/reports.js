...
import { completeDisbursementTask, delayDisbursementTask } from '../../lib/postsales/disbursementTasks.js';
...
router.get('/disbursement-tasks/:unitId', async (req, res) => {

  try {

    const tasks = await DisbursementTask.find({ unitId: req.params.unitId })
...
router.post('/disbursement-tasks/:taskId/complete', async (req, res) => {

  try {

    const task = await completeDisbursementTask(req.params.taskId, {

      completedAmount: req.body.completedAmount,
...
router.post('/disbursement-tasks/:taskId/delay', async (req, res) => {

  try {

    const task = await delayDisbursementTask(req.params.taskId, req.body.revisedDate, req.body.note);
...
router.put('/forecasts/:unitId', async (req, res) => {
  try {
    ...
    const { doc, settlementSummary } = await upsertForecastPayload(unit._id, req.body || {});
    const demands = await Demand.find({ unitId: unit._id }).lean();
    const row = buildCollectionRegisterRow(unit, unit.customerId, demands, doc);

    res.json({ ok: true, row, settlementSummary });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
