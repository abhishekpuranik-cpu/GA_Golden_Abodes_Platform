...
router.get('/', async (req, res) => {
  try {
    const cacheKey = `dashboard:${cacheKeyFromQuery(req.query)}`;
    ...
    const units = await Unit.find(unitFilter)
      ...
      .select('project unitNumber phase building tower entity overallStatus cxPriority totalCost bookingDate customerId')
      .lean();
    const unitIds = units.map((u) => u._id);
    const activeUnitIds = units.filter((u) => u.overallStatus === 'active').map((u) => u._id);

    const [steps, demands, tickets, forecasts, disbTasks, inProgressSteps] = await Promise.all([
      PipelineStep.find({ unitId: { $in: unitIds }, slaBreach: true })
        .select('unitId stepNumber stepName status')
        .lean(),
      unitIds.length
        ? Demand.find({ unitId: { $in: unitIds } }).lean()
        : [],
      Ticket.find({ unitId: { $in: unitIds }, status: { $nin: ['resolved', 'closed'] } })
        .select('unitId ticketNumber description ackSlaBreach resolutionSlaBreach status')
        .lean(),
      CollectionForecast.find({ unitId: { $in: unitIds } }).lean(),
      DisbursementTask.find({ unitId: { $in: unitIds }, status: { $in: ['open', 'delayed'] } })
        .select('unitId status')
        .lean(),
      activeUnitIds.length
        ? PipelineStep.find({ unitId: { $in: activeUnitIds }, status: 'in_progress' })
          .select('phase unitId')
          .lean()
        : [],
    ]);
    ...
    writeHttpCache(cacheKey, payload, DASH_CACHE_TTL);
    res.json(payload);
  } catch (err) {
