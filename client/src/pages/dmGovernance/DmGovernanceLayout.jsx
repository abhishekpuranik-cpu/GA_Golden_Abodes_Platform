...
export default function DmGovernanceLayout() {
  ...
  const navItems = DM_NAV.filter((n) => {
    ...
    if (n.id === DM_TABS.EXECUTIVE) {
      return allowedTabs.has(DM_TABS.CONSOLIDATED) || allowedTabs.has(DM_TABS.EXECUTIVE) || allowedTabs.has(DM_TABS.DASHBOARD);
    }
  ...
  return (
    <>
      <ModuleFrame
        ...
        breadcrumb="Vault / Business Health"
        heroTitle="Business Health"
        heroSub={<>Golden Abodes · {user?.email || '—'}</>}
        navItems={navItems}
        brandTitle="GA Governance"
        brandSub="Business Health"
      >
