...
export default function Milestones() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    const result = await createMilestone({ ...form, clpPercent: Number(form.clpPercent), completedDate: form.completedDate || new Date() });
    setShowForm(false);
    setForm({ project: 'Golden HQ', tower: '', milestoneName: '', clpPercent: '', completedDate: '', loggedBy: '' });
    setToast(`${result.tasksCreated ?? 0} CLP letter task(s) and ${result.demandsCreated ?? 0} demand row(s) created across ${result.unitsAffected ?? 0} unit(s).`);
    setTimeout(() => setToast(''), 6000);
  };

  const handleTrigger = async (id) => {
    const r = await triggerMilestone(id);
    setToast(`Retried: ${r.tasksCreated ?? 0} task(s), ${r.demandsCreated ?? 0} demand(s) for ${r.unitsAffected ?? 0} unit(s).`);
    setTimeout(() => setToast(''), 6000);
  };
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: '0.9rem', color: 'var(--ps-text-muted)', maxWidth: 720 }}>
        When you log a construction milestone, step 12 <strong>Issue CLP letters</strong> tasks are created immediately for eligible units and appear in My Tasks for the backend assignee.
      </p>
      ...
      {showForm && (
        <form className="ps-card" onSubmit={submit}>
          ...
          <div className="ps-form-group"><label>Tower</label><input value={form.tower} onChange={(e) => setForm((f) => ({ ...f, tower: e.target.value }))} placeholder="Optional — limits to one building" /></div>
          <div className="ps-form-group"><label>Milestone name</label><input required value={form.milestoneName} onChange={(e) => setForm((f) => ({ ...f, milestoneName: e.target.value }))} placeholder="e.g. Terrace 15th floor" /></div>
          ...
          <button type="submit" className="ps-btn ps-btn-primary">Log &amp; create CLP letter tasks</button>
        </form>
      )}
        ...
        <div key={status}>
          ...
          {(grouped[status] || []).map((m) => (
            <div key={m._id} className="ps-card" style={status === 'pending' ? { background: 'var(--ps-warning-bg)' } : {}}>
              <strong>{m.project}{m.tower ? ` · ${m.tower}` : ''}</strong> — {m.milestoneName}
              <div style={{ fontSize: '0.85rem' }}>
                CLP {m.clpPercent}% · {m.completedDate ? new Date(m.completedDate).toLocaleDateString('en-IN') : '—'}
                {(m.tasksCreated > 0 || m.demandsCreated > 0) && (
                  <span> · {m.tasksCreated || 0} task(s) · {m.demandsCreated || 0} demand(s)</span>
                )}
              </div>
              {status === 'pending' && (
                <button type="button" className="ps-btn" style={{ marginTop: 8 }} onClick={() => handleTrigger(m._id)}>
                  Retry CLP workflow
                </button>
              )}
            </div>
