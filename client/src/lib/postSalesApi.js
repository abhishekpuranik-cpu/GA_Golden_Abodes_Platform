...
export const postSalesApi = {
  ...
  setUnitMilestoneAchieved: (unitId, milestoneName, achievedDate) => apiFetch(`${BASE}/units/${unitId}/milestone-achieved`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ milestoneName, achievedDate }),
  }).then((r) => { if (!r.ok) throw new Error(r.data?.error); return r.data; }),
  ...
  completeAllClpLetterChecklist: (id, body = {}) => apiFetch(`${BASE}/clp-letter-tasks/${id}/checklist/complete-all`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then((r) => { if (!r.ok) throw new Error(r.data?.error); return r.data; }),
  addClpLetterTaskComment: (id, body) => apiFetch(`${BASE}/clp-letter-tasks/${id}/comments`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then((r) => { if (!r.ok) throw new Error(r.data?.error); return r.data; }),
  ...
  syncClpAchievedDates: (body) => apiFetch(`${BASE}/milestones/clp-schedule/sync-achieved`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then((r) => { if (!r.ok) throw new Error(r.data?.error); return r.data; }),
  ...
  uploadClpScheduleExcel: async (project, file, { phase, building } = {}) => {
    const fd = new FormData();
    fd.append('file', file);
